/**
 * iso-country-codes.ts — Mapeo ISO 3166-1 numérico → alpha-2.
 *
 * Los features de `world-atlas/countries-110m.json` se identifican por el código
 * numérico ISO 3166-1 (feature.id, p.ej. "643" = Rusia). El backend del informe
 * técnico agrega por alpha-2 (`src_country`, p.ej. "RU"). Este mapa une ambos
 * para colorear el mapa mundial (choropleth) por país.
 *
 * Códigos no estándar de world-atlas (Kosovo, N. Chipre, Somalilandia → "-99")
 * simplemente no mapean y quedan sin color, lo cual es correcto.
 */
export const ISO_NUM_TO_A2: Record<string, string> = {
  ...
  "203": "CZ", "204": "BJ", "208": "DK", "212": "DM", "214": "DO", "218": "EC",
  "222": "SV", "226": "GQ", "231": "ET", "232": "ER", "233": "EE", "238": "FK",
  ...
  "508": "MZ", "512": "OM", "516": "NA", "524": "NP", "528": "NL", "533": "AW",
...
/**
 * Escala de calor secuencial (claro → rojo profundo) para el choropleth.
 * `value` se normaliza contra `max` con escala log para discriminar mejor la
 * cola larga. Países sin datos devuelven `null` (el caller pinta neutro).
 *
 * @returns string rgb(...) o null si value<=0.
 */
export function heatColor(value: number, max: number): string | null {
  if (!Number.isFinite(value) || value <= 0 || max <= 0) return null;
  const t = Math.min(1, Math.log1p(value) / Math.log1p(max));
  // Interpolación de [253,230,138] (ámbar claro) → [127,29,29] (rojo profundo).
  const lerp = (a: number, b: number) => Math.round(a + (b - a) * t);
  const r = lerp(253, 127);
  const g = lerp(230, 29);
  const b = lerp(138, 29);
  return `rgb(${r}, ${g}, ${b})`;
}
