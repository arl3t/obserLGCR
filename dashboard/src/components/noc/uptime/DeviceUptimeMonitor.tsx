...
export function DeviceUptimeMonitor({
  ...
  const upSince = device.last_seen_at && device.status === "online"
      ...
      <div className="ut-layout">
        <div className="ut-main">
          <section className="ut-metrics" aria-labelledby="device-status">
            <h2 id="device-status" className="ut-visually-hidden">Métricas de estado</h2>

            <article className="ut-card">
              <p className="ut-card__label">Current Status</p>
              <p className={`ut-metric__value ut-metric__value--${st.tone === "muted" ? "success" : st.tone}`}>
                {st.label}
              </p>
            ...
            <article className="ut-card">
              <p className="ut-card__label">Last check</p>
              <p className="ut-metric__value">{formatAgo(device.last_seen_at)}</p>
              <p className="ut-metric__sub">
                Timeout <strong>{device.heartbeat_timeout_secs}s</strong> · chequeo cada <strong>5 min</strong>
              </p>
            </article>

            <article className="ut-card">
              <p className="ut-card__label">Last 24 hours</p>
              <p className={`ut-metric__value ${pct24 >= 99.5 ? "ut-metric__value--success" : "ut-metric__value--warning"}`}>
                {pct24}%
              </p>
          ...
          <section aria-labelledby="device-history">
            <h2 id="device-history" className="ut-visually-hidden">Historial</h2>
            <div className="ut-history">
              <article className="ut-card">
                <p className="ut-card__label">Last 7 days</p>
                <p className={`ut-history__value ${w7.pct >= 99.5 ? "ut-metric__value--success" : "ut-metric__value--warning"}`}>
                  {w7.pct}%
                </p>
                <p className="ut-history__detail">{w7.incidents} incidente{w7.incidents !== 1 ? "s" : ""}</p>
              </article>
              <article className="ut-card">
                <p className="ut-card__label">Last 30 days</p>
                <p className={`ut-history__value ${w30.pct >= 99 ? "ut-metric__value--success" : "ut-metric__value--warning"}`}>
                  {w30.pct}%
                </p>
                <p className={`ut-history__detail ${w30.incidents > 0 ? "ut-history__detail--warning" : ""}`}>
                  {w30.incidents > 0
              ...
              <article className="ut-card">
                ...
                <p className={`ut-history__value ${alerts.filter((a) => a.status === "open").length > 0 ? "ut-metric__value--warning" : "ut-metric__value--success"}`}>
            ...
            <div className="ut-mtbf" role="status">
              <p className="ut-mtbf__label">Mean Time Between Failures (MTBF)</p>
              <p className="ut-mtbf__value">{mtbf != null ? `${mtbf} days` : "—"}</p>
            </div>
          </section>
