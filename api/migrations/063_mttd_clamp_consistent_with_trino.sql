-- =============================================================================
-- Migration 063 — MTTD: clamp 24h, consistente con Trino lh.incidents.mttd
-- =============================================================================
-- Problema: soc_kpis_window devolvía mttd_min ≈ 42997 (≈30 días) sobre 2,589
-- muestras en 7d. detected_at se popula con el timestamp del log original; en
-- la práctica los IPs reaparecen días o semanas después de su primera
-- observación, por lo que `created_at - detected_at` mide desfase histórico,
-- no MTTD operacional.
--
-- Trino (incident-scoring-sql.mjs:947) clampea el delta entre 0 y 86400s (24h)
-- y devuelve NULL fuera de rango — eso es lo que la UI usa como fuente
-- preferida (SocKpiPanel.tsx:598: mttdTrino ?? k?.mttdMin). Sin clamp en PG,
-- cuando Trino devuelve NULL el fallback PG entrega un número irreal.
--
-- Solución: aplicar el mismo clamp [0, 86400s] al SQL PG → ambas fuentes
-- devuelven NULL honestamente cuando no hay muestras razonables, y devuelven
-- números comparables cuando sí las hay. n_mttd refleja sólo muestras válidas.
--
-- Contrato: misma firma. Sólo cambian dos filtros (mttd_min y n_mttd).
-- =============================================================================
...
RETURNS TABLE (
  ...
  SELECT
    ...
    -- MTTD con clamp [0, 24h] — paridad con Trino lh.incidents.mttd
    ROUND(AVG(EXTRACT(EPOCH FROM created_at - detected_at) / 60.0)
          FILTER (WHERE in_w
                    AND detected_at IS NOT NULL
                    AND detected_at < created_at
    ...
    -- n_mttd refleja muestras post-clamp
...
COMMENT ON FUNCTION soc_kpis_window(int) IS
  'KPIs SOC con ventana operacional dinámica (p_hours). MTTD clampeado a 24h '
  'para paridad con Trino lh.incidents.mttd. MTTC unifica CONTAINMENT '
  'explícito + auto-cierres por bloqueo de firewall. coverage_by_source '
  'devuelve cobertura MITRE por source_log. MITRE coverage y postmortem '
  'rate fijos a 30d.';
