-- =============================================================================
-- Migration 068 — MTTR: filtrar por "cerrado en la ventana", no creado
-- =============================================================================
-- Audit 2026-05-27: post-backfill 067 hay 308 cierres manuales totales pero
-- 0 creados en los últimos 7d. MTTR seguía reportando n=0 porque el filtro
-- in_w aplica a `created_at >= now() - p_hours`, no a updated_at/resolved_at.
--
-- Semántica original: "MTTR de casos creados esta semana y ya resueltos".
-- Semántica NIST/MITRE: "MTTR de los casos resueltos esta semana".
-- La segunda es la convención operacional: si en la ventana actual se cerraron
-- 80 casos con tiempo medio 36d, esa es la realidad — aunque los casos sean
-- viejos. El panel debe reflejar la actividad de cierre, no la cohort de
-- creación.
--
-- Cambio: MTTR (y su n_mttr) ahora usan `closed_in_w` = updated_at >= now() -
-- p_hours. El resto de las métricas mantiene `in_w` sobre created_at — son
-- métricas de "lo que apareció esta ventana" (MTTD, MTTA, FPR, SLA…).
--
-- Esperado post-mig: MTTR alto (~36d) durante la fase actual del SOC, lo cual
-- es genuino backlog operacional. Operacionalmente útil para alertar que
-- el tiempo de cierre supera el SLA.
-- =============================================================================
...
RETURNS TABLE (
  ...
  base AS (
    SELECT *,
           ...
           -- Nueva ventana: actividad de cierre. Usada por MTTR.
  ...
  SELECT
    ...
    -- MTTR ahora usa closed_in_w: casos resueltos en la ventana (no creados).
...
COMMENT ON FUNCTION soc_kpis_window(int) IS
  ...
  'MTTR usa closed_in_w (=updated_at en ventana) — mide actividad de cierre, '
  'no cohort de creación (068).';
