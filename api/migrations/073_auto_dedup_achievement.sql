-- 073_auto_dedup_achievement.sql
-- P4 dedup-churn (auditoría 2026-06-04): auto_dedup_pct usaba el proxy
-- (1 - distinct(ioc_value)/total) que mide la OPORTUNIDAD de dedup (cuántos IOCs
-- se repiten), no el LOGRO (cuántas detecciones se absorbieron en un caso
-- existente). Con occurrence_count ya poblado por P2 (soft-merge en
-- suppress-skip), pasamos a la métrica de logro:
--     (sum(occurrence_count) - count(casos)) / sum(occurrence_count) * 100
--   = fracción de detecciones deduplicadas sobre el total de detecciones.
-- Empieza ~0% (honesto: aún no se deduplicaba) y sube a medida que P2 acumula.
-- Requiere exponer occurrence_count en los CTE base90/base.
...
WITH base90 AS (
 ...
 SELECT count(*) FILTER (WHERE status::text <> ALL (ARRAY['CERRADO'::character varying, 'FALSO_POSITIVO'::character varying]::text[])) AS open_cases,
    ...
    round(
        CASE
            WHEN COALESCE(sum(COALESCE(occurrence_count, 1)) FILTER (WHERE in_w7), 0::bigint) = 0 THEN NULL::numeric
            ELSE sum(COALESCE(occurrence_count, 1) - 1) FILTER (WHERE in_w7)::numeric * 100.0 / NULLIF(sum(COALESCE(occurrence_count, 1)) FILTER (WHERE in_w7), 0)::numeric
